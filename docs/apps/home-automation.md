...
The production Thread border router is OTBR in Kubernetes. The SLZB-MR3U
provides two independent network radios; it does not run OTBR and does not use
multiprotocol firmware.

```text
SLZB-MR3U Radio 2 (CC2674P10) -- TCP --> Zigbee2MQTT --> Mosquitto
SLZB-MR3U Radio 1 (EFR32MG24)  -- TCP --> Kubernetes OTBR
                                                   |
...
1. Give the MR3U a stable DHCP reservation and make
   `slzb-mr3u.home.arpa` resolve from every cluster node.
2. Configure each radio independently in the MR3U UI and record the endpoints
...
- Radio 2, CC2674P10: Zigbee coordinator using the Zigbee2MQTT `zstack`
  adapter.
- Radio 1, EFR32MG24: dedicated OpenThread RCP connected to Kubernetes OTBR.

Keep Zigbee and Thread on separate radios. Do not install single-radio
multiprotocol firmware. Do not enable the MR3U's beta built-in OTBR while the
...
Replace `tcp://slzb-mr3u.home.arpa:0` with the exact Radio 2 Zigbee endpoint
shown by the MR3U. Leave `serial_adapter: zstack`; change `serial_baudrate` only
...
Set `RCP_PORT` to the exact EFR32MG24 Thread RCP endpoint exposed by the MR3U
...
- required mDNS reflection and SSDP/UPnP discovery between trusted and IoT
  networks;
- `thinkcentre-01` to the MR3U Thread RCP TCP endpoint;
- Zigbee2MQTT pods to the MR3U Zigbee TCP endpoint;
...
Use this only when the MR3U's built-in OTBR is considered stable. Never create
...
2. Stop/disable the Kubernetes OTBR Deployment and verify it has released the
   RCP before enabling built-in OTBR on the MR3U.
3. Enable MR3U OTBR and import the same active Thread dataset. Do not create a
...
4. Change only Home Assistant's OpenThread Border Router integration URL to
   `http://slzb-mr3u.home.arpa:<OTBR_PORT>`; Matter Server continues using LAN
...
Rollback by stopping MR3U OTBR before restarting Kubernetes OTBR with its
